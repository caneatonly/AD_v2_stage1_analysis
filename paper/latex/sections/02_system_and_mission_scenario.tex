...
\section{Platform description and deployment scenario}
...
For the nominal setup used throughout this manuscript, key physical parameters are frozen in the project parameter file: dry mass $m_{dry}=\SI{2.55}{kg}$, wet mass $m_{wet}=\SI{2.76}{kg}$, inner-water equivalent mass $m_{water,inner}=\SI{0.21}{kg}$, pitch inertia $I_{yy}=\SI{0.05741}{kg\,m^2}$, and inner-water inertia $I_{water,inner}=\SI{0.01119}{kg\,m^2}$. Buoyancy is implemented as equivalent mass with $B_{mass}=\SI{2.55}{kg}$ and nominal offset $x_b=\SI{0.02535}{m}$. These values define the baseline configuration for all reported simulation-experiment comparisons.
...
\subsection{Phase of interest and state definition}
...
The state interface used across data processing, modeling, and simulation is fixed as $\nu=[u,w,q]^T$ and $\eta=[\theta]$. Sign and axis conventions are frozen as project-level definitions: body $x_b$ forward, $z_b$ downward, nose-up pitch positive, and $q=d\theta/dt$ about $+y_b$. Angle of attack is computed as $\alpha=\mathrm{atan2}(w,u)$. Freezing these definitions prevents silent inconsistency between CFD tables, identification scripts, and dynamic simulation.
