...
\section{Anisotropic permeability-corrected dynamics}

\subsection{Governing equations and state definition}

This section introduces the 3-DOF governing model for the horizontal-launch to vertical-stabilization phase, with frozen interfaces $\nu=[u,w,q]^T$ and $\eta=[\theta]$. The body-axis and sign conventions follow Section~2 and \path{sim_flip/src/conventions.py} (Fig.~\ref{fig:conventions}): $x_b$ forward, $z_b$ downward, nose-up pitch positive, and $q=d\theta/dt$ about $+y_b$.
...
\end{align}
where $A_{ref}$ and $L_{ref}$ are fixed reference area and length (\path{sim_flip/configs/params_nominal.yaml}). The attack angle is computed by the frozen convention
\begin{equation}
  \alpha_{raw} = \mathrm{atan2}(w, u),
\end{equation}
and mapped/clamped to the CFD lookup range at runtime (details in Section~4; implementation in \path{sim_flip/src/cfd_table.py} and \path{sim_flip/src/kinematics.py}). Following the equation-governance rule, no additional explicit Munk-moment term is introduced when $C_m(\alpha)$ already encodes the corresponding static effect.

\subsection{Permeability-corrected added-mass terms}
...
All terms above are frozen by the parameter contract and are computed in \path{sim_flip/src/added_mass.py} from \path{sim_flip/configs/params_nominal.yaml} (\texttt{rigid\_body}, \texttt{added\_mass\_outer}, \texttt{permeability}). Consistent with the implementation, the translational denominators use $m_{dry}$ (not $m_{wet}$).

\subsection{Restoring, damping, and actuation terms}
...
\subsection{Model interfaces and reproducibility mapping}

The manuscript equations above map one-to-one to the repository implementation:
\begin{itemize}
  \item State ordering: $y=[u,w,q,\theta]$ is used consistently in the ODE right-hand side and events (\path{sim_flip/src/dynamics.py}).
  \item Kinematics and CFD inputs: $V$, $Q$, and $\alpha$ handling are computed in \path{sim_flip/src/kinematics.py}; coefficient interpolation uses \path{sim_flip/src/cfd_table.py} and the default dataset \path{sim_flip/data/cfd_table_clean.csv}.
  \item Permeability correction: added-mass totals and effective inertias are computed in \path{sim_flip/src/added_mass.py} from the YAML parameter tree.
  \item Parameter interfaces: \texttt{rho/g/A\_ref/L\_ref}, \texttt{m\_dry/Iyy/m\_water\_inner/I\_water\_inner}, \texttt{X\_udot\_outer/Z\_wdot\_outer/M\_qdot\_outer}, \texttt{mu\_x/mu\_z/mu\_theta}, \texttt{B\_mass/x\_b/z\_b}, \texttt{d\_q/d\_q\_abs}, and optional cable terms are single-sourced in \path{sim_flip/configs/params_nominal.yaml}.
\end{itemize}
